...
public class SecurityRestFilter {
    ...
    class AuthczRestHandler extends DelegatingRestHandler {
        ...
        public void handleRequest(RestRequest request, RestChannel channel, NodeClient client) throws Exception {
            final String traceparentFromRequest = request.header(TRACEPARENT_HEADER);
            if (traceparentFromRequest != null) {
                log.info("Security plugin rest request handler started processing request with traceparent = " + traceparentFromRequest);
            }
            ...
            NettyAttribute.popFrom(request, Netty4HttpRequestHeaderVerifier.CONTEXT_TO_RESTORE).ifPresent(storedContext -> {
                ...
                if (traceparentFromRequest != null) {
                    ...
                    threadContext.putHeader(TRACEPARENT_HEADER, traceparentFromContext);
                }
            });
            ...
            if (traceparentFromRequest != null) {
                log.info("Security plugin rest request handler finished processing request with traceparent = " + traceparentFromRequest);
            }
